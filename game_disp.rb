...
module GameDisp
  def create_game
    ...
    @games << { 'id' => game.id, 'multiplayer_game' => game.multiplayer, 'last_played_date' => game.last_played_at,
                'released_date' => game.published_date }
  ...
  def display_games
    ...
    if @games.empty?
      ...
      @games.each do |game|
        ...
        puts "Published Date: #{game['released_date']}"
        puts '......................................................'
        puts ''
      end
    end
  ...
  def preserve_game(game)
    File.write('games.json', JSON.generate(game))
  end
  ...
  def retrieve_game
    ...
    @games = File.empty?('./games.json') ? [] : JSON.parse(File.read('games.json'))
  end

  def retrieve_author
    return unless File.exist?('author.json')

    @authors = File.empty?('./author.json') ? [] : JSON.parse(File.read('author.json'))
  end
end
